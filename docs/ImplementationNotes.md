...
- Fixed Playwright CI regression after CSRF hardening by forcing CSRF token cookie generation in backend security filter chain and aligning frontend axios `xsrfCookieName`/`xsrfHeaderName` with Spring defaults.