...
> ⚠️ **This session is local-only** (remote MCP is forbidden). Use the local
> `.vscode/mcp.json` template and a private registry per
...
> 🆕 **Optional — Step 0: Create the BRD from a business idea.** This walkthrough assumes
> [docs/BRD.md](../../docs/BRD.md) already exists. If it does not (or you want to practice
> authoring requirements), use the interview-style **`@0.SDLC BRD Author`** agent first to
> generate `docs/BRD.md` from a business idea, then continue here. See
> [9.BRD-From-Scratch.ja.md](9.BRD-From-Scratch.ja.md) and the sample brief
> [docs/brd-brief.example.md](../../docs/brd-brief.example.md).
