...
```
【事前課題（前日まで）】
  1. setup/1.Prerequisites.ja.md  →  2. setup/3.MCP-Local-Setup.ja.md

【当日の流れ（このファイルを入口に）】
  3. ここ（Workshop-Guide.ja.md）
        ↓
  4. guides/0.Concepts-Primer.ja.md   （概念理解 35分）
        ↓
  5. guides/5.Demo-Flow.ja.md          （メインデモ実演）
        ↓
  6. guides/11.GHAS-Pipeline.ja.md     （セキュリティ確認）
        ↓
  7. guides/12.QA-Strategy.ja.md       （QA + Playwright）
        ↓
  8. guides/7.Custom-Agents-Exercise.ja.md  （カスタムエージェント演習）🎉
```

> **当日の最初の 1 クリック →** [**概念プライマーを始める**](guides/0.Concepts-Primer.ja.md)
